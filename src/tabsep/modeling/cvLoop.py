...
def do_scoring(dl: torch.utils.data.DataLoader, scorable_model) -> Tuple[float, float]:
    with torch.inference_mode(mode=True):
        ...
        ret = {}
        for idx, label_name in enumerate(dl.dataset.get_label_names()):
            ret[f"auc_{label_name}"] = roc_auc_score(
                y_actual[:, idx].to("cpu"), y_pred[:, idx].to("cpu")
            )
...
def do_cv(
    ...
    if stay_ids is not None:
        all_stay_ids = stay_ids
    else:
        all_stay_ids = pd.read_csv(f"cache/included_stayids.csv").squeeze("columns")
    ...
    # need to load all labels (in order) for stratified multilabel CV
    # TODO: unit test multilabel stratification (verify distributions)
    print("Fetching labels for stratified CV and oversampling")

    all_labels = pd.read_csv(f"cache/labels.csv", index_col=0)
    all_labels = all_labels.reindex(stay_ids)
    all_labels = all_labels.astype("float")
    print(all_labels.values.shape)

    assert all_labels.values.shape[0] == len(stay_ids)
    assert not all_labels["label"].isna().any()

    for cv_idx, (train, test) in enumerate(cv.split(all_stay_ids, all_labels.values)):
        print(f"===== Starting CV fold {cv_idx} =====")

        train_stay_ids = all_stay_ids[train]
        test_stay_ids = all_stay_ids[test]

        resampled_train_stay_ids = train_stay_ids

        train_ds = FileBasedDataset(
            "cache/mimicts", stay_ids=resampled_train_stay_ids.to_list()
        )
        test_ds = FileBasedDataset("cache/mimicts", stay_ids=test_stay_ids.to_list())
